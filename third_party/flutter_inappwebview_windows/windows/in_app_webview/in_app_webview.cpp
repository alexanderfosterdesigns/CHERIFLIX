...
namespace flutter_inappwebview_plugin
{
  ...
  namespace
  {
    const std::string kYouTubeEmbedRefererFallback = "https://preview.cheriflix.local/";

    std::optional<std::string> extract_url_authority(const std::string& url)
    {
      auto schemeSeparatorIndex = url.find("://");
      if (schemeSeparatorIndex == std::string::npos) {
        return std::nullopt;
      }

      auto authorityStartIndex = schemeSeparatorIndex + 3;
      auto authorityEndIndex = url.find_first_of("/?#", authorityStartIndex);
      auto authority = url.substr(
        authorityStartIndex,
        authorityEndIndex == std::string::npos
          ? std::string::npos
          : authorityEndIndex - authorityStartIndex);

      if (authority.empty()) {
        return std::nullopt;
      }

      auto userInfoSeparatorIndex = authority.rfind('@');
      if (userInfoSeparatorIndex != std::string::npos) {
        authority = authority.substr(userInfoSeparatorIndex + 1);
      }

      return authority.empty()
        ? std::optional<std::string>{}
        : std::make_optional(authority);
    }

    std::optional<std::string> extract_url_host(const std::string& url)
    {
      auto authority = extract_url_authority(url);
      if (!authority.has_value()) {
        return std::nullopt;
      }

      auto host = authority.value();
      auto portSeparatorIndex = host.find(':');
      if (portSeparatorIndex != std::string::npos) {
        host = host.substr(0, portSeparatorIndex);
      }
      ...
      return std::make_optional(to_lowercase_copy(host));
    }

    std::optional<std::string> extract_url_origin(const std::string& url)
    {
      auto schemeSeparatorIndex = url.find("://");
      if (schemeSeparatorIndex == std::string::npos) {
        return std::nullopt;
      ...
      return std::make_optional(
        url.substr(0, schemeSeparatorIndex + 3) + authority.value() + "/");
    }
    ...
    bool is_youtube_related_request(const std::string& url)
    {
      ...
      return host_matches_domain(host.value(), "youtube.com") ||
        host_matches_domain(host.value(), "youtube-nocookie.com") ||
        host_matches_domain(host.value(), "ytimg.com") ||
        host_matches_domain(host.value(), "googlevideo.com") ||
        host_matches_domain(host.value(), "youtubei.googleapis.com");
    }

    std::string resolve_embed_referer(ICoreWebView2* sender)
    {
      if (sender) {
        wil::unique_cotaskmem_string source;
        if (SUCCEEDED(sender->get_Source(&source)) && source.get()) {
          auto sourceOrigin = extract_url_origin(wide_to_utf8(source.get()));
      ...
      return kYouTubeEmbedRefererFallback;
    }

    void maybe_inject_youtube_referer_header(
      ...
      ICoreWebView2WebResourceRequest* request)
    {
      ...
      if (FAILED(request->get_Uri(&uri)) || !uri.get()) {
        return;
      }

      const auto url = wide_to_utf8(uri.get());
      if (!is_youtube_related_request(url)) {
        return;
      }
      ...
      const auto referer = utf8_to_wide(resolve_embed_referer(sender));
      failedLog(requestHeaders->SetHeader(L"Referer", referer.c_str()));
    }
  }
      ...
      Callback<ICoreWebView2WebResourceRequestedEventHandler>(
          ...
          ICoreWebView2* sender, ICoreWebView2WebResourceRequestedEventArgs* args)
        {
          ...
          if (channelDelegate && succeededOrLog(args->get_Request(&webResourceRequest)) && succeededOrLog(args->GetDeferral(&deferral))) {
            maybe_inject_youtube_referer_header(sender, webResourceRequest.get());
