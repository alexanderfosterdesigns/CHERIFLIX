...
class VideoControllerConfiguration {
  ...
  /// Optional override for selecting Android video output backend.
  ///
  /// * `true`: force SurfaceTexture backend.
  /// * `false`: force SurfaceProducer backend.
  /// * `null`: use platform default / runtime override logic.
  final bool? androidForceSurfaceTexture;
  ...
  const VideoControllerConfiguration({
    ...
    this.androidForceSurfaceTexture,
  });
  ...
  VideoControllerConfiguration copyWith({
    ...
    bool? androidForceSurfaceTexture,
  }) =>
      VideoControllerConfiguration(
        vo: vo ?? this.vo,
        ...
        androidAttachSurfaceAfterVideoParameters:
            androidAttachSurfaceAfterVideoParameters ??
                this.androidAttachSurfaceAfterVideoParameters,
        androidForceSurfaceTexture:
            androidForceSurfaceTexture ?? this.androidForceSurfaceTexture,
      );

  /// Resolves whether surface attachment should wait for video parameters.
  ///
  /// If [androidAttachSurfaceAfterVideoParameters] is explicitly set, that
  /// value is used. Otherwise this mirrors package defaults:
  /// * `true` for `vo=gpu`
  /// * `false` for other `vo` values.
  bool get shouldAttachSurfaceAfterVideoParameters {
    final explicit = androidAttachSurfaceAfterVideoParameters;
    if (explicit != null) {
      return explicit;
    }
    final effectiveVo = (vo ?? 'gpu').trim().toLowerCase();
    return effectiveVo == 'gpu';
  }
}
