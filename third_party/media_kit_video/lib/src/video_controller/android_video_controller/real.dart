/// This file is a part of media_kit (https://github.com/media-kit/media-kit).
///
/// Copyright © 2021 & onwards, Hitesh Kumar Saini <saini123hitesh@gmail.com>.
/// All rights reserved.
/// Use of this source code is governed by MIT license that can be found in the LICENSE file.
import 'dart:io';
import 'dart:async';
import 'dart:collection';
import 'package:flutter/services.dart';
import 'package:flutter/foundation.dart';
import 'package:synchronized/synchronized.dart';

import 'package:media_kit/media_kit.dart';

import 'package:media_kit_video/src/utils/query_decoders.dart';
import 'package:media_kit_video/src/video_controller/platform_video_controller.dart';

bool? _parseSurfaceTextureOverrideValue(String? value) {
  final normalized = value?.trim().toLowerCase();
  if (normalized == null || normalized.isEmpty) {
    return null;
  }
  if (normalized == '1' ||
      normalized == 'true' ||
      normalized == 'yes' ||
      normalized == 'on') {
    return true;
  }
  if (normalized == '0' ||
      normalized == 'false' ||
      normalized == 'no' ||
      normalized == 'off') {
    return false;
  }
  return null;
}

@visibleForTesting
Map<String, Object> buildAndroidVideoOutputManagerCreateArguments({
  required int handle,
  required bool forceSurfaceTexture,
  required bool attachSurfaceAfterVideoParameters,
}) {
  return <String, Object>{
    'handle': handle.toString(),
    'forceSurfaceTexture': forceSurfaceTexture,
    'attachSurfaceAfterVideoParameters': attachSurfaceAfterVideoParameters,
  };
}

@visibleForTesting
bool shouldPrimeInitialAndroidSurfaceSize({
  required bool attachSurfaceAfterVideoParameters,
}) {
  return !attachSurfaceAfterVideoParameters;
}

@visibleForTesting
bool shouldBindAndroidSurfaceFromTextureUpdate({
  required int previousWid,
  required int nextWid,
  required int width,
  required int height,
  required int previousWidth,
  required int previousHeight,
}) {
  if (nextWid == 0 || width <= 0 || height <= 0) {
    return false;
  }
  return previousWid != nextWid ||
      previousWidth != width ||
      previousHeight != height;
}

@visibleForTesting
bool shouldRefreshAndroidVideoAfterSurfaceBind({
  required String? videoOutputName,
  required bool attachSurfaceAfterVideoParameters,
  required int previousBoundWid,
  required int nextWid,
}) {
  if (previousBoundWid != 0 || nextWid == 0) {
    return false;
  }
  if (attachSurfaceAfterVideoParameters) {
    return true;
  }
  return videoOutputName?.trim().toLowerCase() == 'mediacodec_embed';
}

/// {@template android_video_controller}
///
/// AndroidVideoController
/// ----------------------
///
/// The [PlatformVideoController] implementation based on native JNI & C/C++ used on Android.
///
/// {@endtemplate}
class AndroidVideoController extends PlatformVideoController {
  /// Whether [AndroidVideoController] is supported on the current platform or not.
  static bool get supported => Platform.isAndroid;

  static final bool _forceSurfaceTextureDefault =
      _parseSurfaceTextureOverrideValue(
            const String.fromEnvironment(
              'CHERIFLIX_MEDIA_KIT_FORCE_SURFACE_TEXTURE',
            ),
          ) ??
          _parseSurfaceTextureOverrideValue(
            const String.fromEnvironment(
              'MEDIA_KIT_VIDEO_FORCE_SURFACE_TEXTURE',
            ),
          ) ??
          _parseSurfaceTextureOverrideValue(
            Platform.environment['CHERIFLIX_MEDIA_KIT_FORCE_SURFACE_TEXTURE'],
          ) ??
          _parseSurfaceTextureOverrideValue(
            Platform.environment['MEDIA_KIT_VIDEO_FORCE_SURFACE_TEXTURE'],
          ) ??
          true;

  /// Pointer address to the global object reference of `android.view.Surface` i.e. `(intptr_t)(*android.view.Surface)`.
  final ValueNotifier<int?> wid = ValueNotifier<int?>(null);

  int _lastBoundWid = 0;
  int _lastBoundWidth = 0;
  int _lastBoundHeight = 0;
  bool _deferredSurfaceRefreshScheduled = false;

  /// [Lock] used to synchronize [onLoadHooks], [onUnloadHooks] & [subscription].
  final lock = Lock();

  NativePlayer get platform => player.platform as NativePlayer;

  Future<void> setProperty(String key, String value) async {
    await platform.setProperty(key, value, waitForInitialization: false);
  }

  Future<void> setProperties(Map<String, String> properties) async {
    for (final entry in properties.entries) {
      await setProperty(entry.key, entry.value);
    }
  }

  Future<void> _bindSurfaceToMpv({
    required int width,
    required int height,
  }) async {
    final normalizedWidth = width <= 0 ? 1 : width;
    final normalizedHeight = height <= 0 ? 1 : height;
    final androidSurfaceSizeValue =
        [normalizedWidth, normalizedHeight].join('x');
    final widValue = wid.value?.toString() ?? '0';
    // When --wid is 0, vo=null is required to avoid SIGSEGV.
    final voValue = widValue == '0' ? 'null' : configuration.vo!;
    final vidValue = widValue == '0' ? 'no' : 'auto';
    // It is important to re-initialize --vo after --android-surface-size.
    await setProperty('vo', 'null');
    await setProperties(
      {
        // ORDER IS IMPORTANT.
        'android-surface-size': androidSurfaceSizeValue,
        'wid': widValue,
        'vo': voValue,
        // It is important to re-initialize --vid in-case of --vo=mediacodec_embed.
        // Not doing so causes error "Could not open codec." & video never gets rendered.
        if (configuration.vo == 'mediacodec_embed') 'vid': vidValue,
      },
    );
    // Re-seek only for mediacodec_embed to refresh decoder binding after
    // WID changes. Doing this for all outputs can rewind startup/resume when
    // player.state.position is still stale (often near zero).
    if (configuration.vo == 'mediacodec_embed') {
      final currentPosition = player.state.position;
      if (currentPosition > Duration.zero) {
        await player.seek(currentPosition);
      }
    }
  }

  Future<void> _refreshVideoAfterDeferredSurfaceBind({
    required String reason,
  }) async {
    var currentPosition = player.state.position;
    if (currentPosition <= Duration.zero) {
      try {
        currentPosition = await player.stream.position
            .firstWhere((position) => position > Duration.zero)
            .timeout(
              const Duration(seconds: 2),
              onTimeout: () => player.state.position,
            );
      } catch (_) {
        currentPosition = player.state.position;
      }
    }
    debugPrint(
      'media_kit: AndroidVideoController refresh video after deferred '
      'surface bind reason=$reason position=$currentPosition',
    );
    await player.seek(currentPosition);
  }

  void _scheduleRefreshVideoAfterDeferredSurfaceBind({
    required String reason,
  }) {
    if (_deferredSurfaceRefreshScheduled) {
      return;
    }
    _deferredSurfaceRefreshScheduled = true;
    unawaited(() async {
      try {
        await _refreshVideoAfterDeferredSurfaceBind(reason: reason);
      } finally {
        _deferredSurfaceRefreshScheduled = false;
      }
    }());
  }

  Future<bool> _maybeBindSurfaceToMpv({
    required int width,
    required int height,
    required String reason,
    bool force = false,
  }) async {
    final normalizedWid = wid.value ?? 0;
    final normalizedWidth = width <= 0 ? 1 : width;
    final normalizedHeight = height <= 0 ? 1 : height;
    if (!force &&
        normalizedWid == _lastBoundWid &&
        normalizedWidth == _lastBoundWidth &&
        normalizedHeight == _lastBoundHeight) {
      return false;
    }
    final previousBoundWid = _lastBoundWid;
    debugPrint(
      'media_kit: AndroidVideoController bind surface '
      'reason=$reason wid=$normalizedWid '
      'size=${normalizedWidth}x$normalizedHeight',
    );
    await _bindSurfaceToMpv(
      width: normalizedWidth,
      height: normalizedHeight,
    );
    _lastBoundWid = normalizedWid;
    _lastBoundWidth = normalizedWidth;
    _lastBoundHeight = normalizedHeight;
    if (shouldRefreshAndroidVideoAfterSurfaceBind(
      videoOutputName: configuration.vo,
      attachSurfaceAfterVideoParameters:
          configuration.shouldAttachSurfaceAfterVideoParameters,
      previousBoundWid: previousBoundWid,
      nextWid: normalizedWid,
    )) {
      _scheduleRefreshVideoAfterDeferredSurfaceBind(reason: reason);
    }
    return true;
  }

  void _resetSurfaceBindingCache() {
    _lastBoundWid = 0;
    _lastBoundWidth = 0;
    _lastBoundHeight = 0;
  }

  Future<void> _handleTextureResizeEvent({
    required Rect nextRect,
    required int nextId,
    required int nextWid,
  }) async {
    final previousWid = wid.value ?? 0;
    final previousRect = rect.value;
    rect.value = nextRect;
    id.value = nextId;
    wid.value = nextWid;

    if (nextWid == 0) {
      _resetSurfaceBindingCache();
      return;
    }

    final width = nextRect.width.toInt();
    final height = nextRect.height.toInt();
    final previousWidth = previousRect?.width.toInt() ?? 0;
    final previousHeight = previousRect?.height.toInt() ?? 0;
    if (!shouldBindAndroidSurfaceFromTextureUpdate(
      previousWid: previousWid,
      nextWid: nextWid,
      width: width,
      height: height,
      previousWidth: previousWidth,
      previousHeight: previousHeight,
    )) {
      return;
    }

    await lock.synchronized(() async {
      await _maybeBindSurfaceToMpv(
        width: width,
        height: height,
        reason: 'texture-resize',
      );
    });
  }

  /// Listener for updating the --wid property.
  Future<void> widListener() {
    return lock.synchronized(() async {
      final width = rect.value?.width.toInt() ?? 1;
      final height = rect.value?.height.toInt() ?? 1;
      await _maybeBindSurfaceToMpv(
        width: width,
        height: height,
        reason: 'wid-listener',
      );
    });
  }

  /// [StreamSubscription] for listening to video [Rect].
  StreamSubscription<VideoParams>? videoParamsSubscription;

  /// {@macro android_video_controller}
  AndroidVideoController._(
    super.player,
    super.configuration,
  ) {
    wid.addListener(widListener);
    videoParamsSubscription = player.stream.videoParams.listen(
      (event) => lock.synchronized(() async {
        final int width;
        final int height;
        if (event.rotate == 0 || event.rotate == 180) {
          width = event.dw ?? 0;
          height = event.dh ?? 0;
        } else {
          // width & height are swapped for 90 or 270 degrees rotation.
          width = event.dh ?? 0;
          height = event.dw ?? 0;
        }

        final isZero = width == 0 || height == 0;
        final isSame = width == rect.value?.width.toInt() &&
            height == rect.value?.height.toInt();
        if (isZero || isSame) {
          return;
        }

        final handle = await player.handle;

        await _channel.invokeMethod(
          'VideoOutputManager.SetSurfaceSize',
          {
            'handle': handle.toString(),
            'width': width.toString(),
            'height': height.toString(),
          },
        );

        rect.value = Rect.fromLTWH(
          0.0,
          0.0,
          width.toDouble(),
          height.toDouble(),
        );

        if (!waitUntilFirstFrameRenderedCompleter.isCompleted) {
          waitUntilFirstFrameRenderedCompleter.complete();
        }
        if (configuration.shouldAttachSurfaceAfterVideoParameters) {
          if ((wid.value ?? 0) != 0) {
            await _maybeBindSurfaceToMpv(
              width: width,
              height: height,
              reason: 'video-params-with-wid',
            );
          }
        } else {
          await _maybeBindSurfaceToMpv(
            width: width,
            height: height,
            reason: 'video-params',
          );
        }
      }),
    );
  }

  /// {@macro android_video_controller}
  static Future<PlatformVideoController> create(
    Player player,
    VideoControllerConfiguration configuration,
  ) async {
    final bool isEmulator = await _channel.invokeMethod('Utils.IsEmulator');
    final requestedVo = configuration.vo ?? 'gpu';

    Future<String> resolveHwdec(String vo) async {
      final normalizedVo = vo.trim().toLowerCase();
      if (isEmulator && normalizedVo == 'gpu') {
        debugPrint('media_kit: Emulator detected.');
        debugPrint('media_kit: Enforcing S/W rendering.');
        return 'no';
      }
      final hw = configuration.enableHardwareAcceleration;
      return configuration.hwdec ?? (hw ? 'auto-safe' : 'no');
    }

    // Update [configuration] to have default values.
    configuration = configuration.copyWith(
      vo: requestedVo,
      hwdec: await resolveHwdec(requestedVo),
    );
    final attachSurfaceAfterVideoParameters =
        configuration.shouldAttachSurfaceAfterVideoParameters;
    final forceSurfaceTexture =
        configuration.androidForceSurfaceTexture ?? _forceSurfaceTextureDefault;
    debugPrint(
      'media_kit: AndroidVideoController create: '
      'vo=${configuration.vo} '
      'hwdec=${configuration.hwdec} '
      'attachAfterParams=$attachSurfaceAfterVideoParameters '
      'forceSurfaceTexture=$forceSurfaceTexture',
    );

    // Retrieve the native handle of the [Player].
    final handle = await player.handle;
    // Return the existing [VideoController] if it's already created.
    if (_controllers.containsKey(handle)) {
      return _controllers[handle]!;
    }

    // In case no video-decoders are found, this means media_kit_libs_***_audio is being used.
    // Thus, --vid=no is required to prevent libmpv from trying to decode video (otherwise bad things may happen).
    //
    // Search for common H264 decoder to check if video support is available.
    final decoders = await queryDecoders(handle);
    if (!decoders.contains('h264')) {
      throw UnsupportedError(
        '[VideoController] is not available.'
        ' '
        'Please use media_kit_libs_***_video instead of media_kit_libs_***_audio.',
      );
    }

    // Creation:
    final controller = AndroidVideoController._(
      player,
      configuration,
    );

    // Register [_dispose] for execution upon [Player.dispose].
    player.platform?.release.add(controller._dispose);

    // Store the [VideoController] in the [_controllers].
    _controllers[handle] = controller;

    await _channel.invokeMethod(
      'VideoOutputManager.Create',
      buildAndroidVideoOutputManagerCreateArguments(
        handle: handle,
        forceSurfaceTexture: forceSurfaceTexture,
        attachSurfaceAfterVideoParameters: attachSurfaceAfterVideoParameters,
      ),
    );

    if (shouldPrimeInitialAndroidSurfaceSize(
      attachSurfaceAfterVideoParameters: attachSurfaceAfterVideoParameters,
    )) {
      await _channel.invokeMethod(
        'VideoOutputManager.SetSurfaceSize',
        {
          'handle': handle.toString(),
          'width': '1',
          'height': '1',
        },
      );
    }

    await controller.setProperties(
      {
        // It is necessary to set vo=null here to avoid SIGSEGV, --wid must be assigned before vo=gpu is set.
        'vo': 'null',
        'hwdec': configuration.hwdec!,
        'vid': 'auto',
        'opengl-es': 'yes',
        if (isEmulator && configuration.vo == 'gpu') 'gpu-sw': 'yes',
        'force-window': 'yes',
        'gpu-context': 'android',
        'sub-use-margins': 'no',
        'sub-font-provider': 'none',
        'sub-scale-with-window': 'yes',
        'hwdec-codecs': 'h264,hevc,mpeg4,mpeg2video,vp8,vp9,av1',
      },
    );

    // Return the [PlatformVideoController].
    return controller;
  }

  /// Sets the required size of the video output.
  /// This may yield substantial performance improvements if a small [width] & [height] is specified.
  ///
  /// Remember:
  /// * “Premature optimization is the root of all evil”
  /// * “With great power comes great responsibility”
  @override
  Future<void> setSize({
    int? width,
    int? height,
  }) {
    throw UnsupportedError(
      '[AndroidVideoController.setSize] is not available on Android',
    );
  }

  /// Returns native Android video output diagnostics for render-health probing.
  Future<Map<String, dynamic>?> debugVideoOutputState() async {
    final handle = await player.handle;
    final response = await _channel.invokeMethod<dynamic>(
      'VideoOutputManager.GetState',
      {
        'handle': handle.toString(),
      },
    );
    if (response is! Map) {
      return null;
    }
    return response.map(
      (key, value) => MapEntry(key.toString(), value),
    );
  }

  /// Disposes the instance. Releases allocated resources back to the system.
  Future<void> _dispose() async {
    super.dispose();
    _resetSurfaceBindingCache();
    wid.dispose();
    wid.removeListener(widListener);
    await videoParamsSubscription?.cancel();
    final handle = await player.handle;
    _controllers.remove(handle);
    await _channel.invokeMethod(
      'VideoOutputManager.Dispose',
      {
        'handle': handle.toString(),
      },
    );
  }

  /// Currently created [AndroidVideoController]s.
  static final _controllers = HashMap<int, AndroidVideoController>();

  /// [MethodChannel] for invoking platform specific native implementation.
  static final _channel =
      const MethodChannel('com.alexmercerind/media_kit_video')
        ..setMethodCallHandler(
          (MethodCall call) async {
            try {
              debugPrint(call.method.toString());
              debugPrint(call.arguments.toString());
              switch (call.method) {
                case 'VideoOutput.Resize':
                  {
                    // Notify about updated texture ID & [Rect].
                    final int handle = call.arguments['handle'];
                    final Rect rect = Rect.fromLTWH(
                      call.arguments['rect']['left'] * 1.0,
                      call.arguments['rect']['top'] * 1.0,
                      call.arguments['rect']['width'] * 1.0,
                      call.arguments['rect']['height'] * 1.0,
                    );
                    final int id = call.arguments['id'];
                    final int wid = call.arguments['wid'];
                    final controller = _controllers[handle];
                    if (controller != null) {
                      await controller._handleTextureResizeEvent(
                        nextRect: rect,
                        nextId: id,
                        nextWid: wid,
                      );
                    }
                    break;
                  }
                case 'VideoOutput.WaitUntilFirstFrameRenderedNotify':
                  {
                    // Notify about updated texture ID & [Rect].
                    final int handle = call.arguments['handle'];
                    debugPrint(handle.toString());
                    // Notify about the first frame being rendered.
                    final completer = _controllers[handle]
                        ?.waitUntilFirstFrameRenderedCompleter;
                    if (!(completer?.isCompleted ?? true)) {
                      completer?.complete();
                    }
                    break;
                  }
                default:
                  {
                    break;
                  }
              }
            } catch (exception, stacktrace) {
              debugPrint(exception.toString());
              debugPrint(stacktrace.toString());
            }
          },
        );
}
